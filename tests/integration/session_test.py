...
async def test_async_create(
    ...
    assert isinstance(await async_session.create(version=version), expected)
...
async def test_async_create_with_wrong_account(
    ...
    with pytest.raises(ig.InvalidDetailsError):
        await async_session.create(identifier="foo")
...
async def test_async_create_with_wrong_password(
    ...
    with pytest.raises(ig.InvalidDetailsError):
        await async_session.create(password="foo")
...
async def test_async_refresh_token_create(
    ...
    account: t.Final = await async_session.create()
    assert isinstance(
        await async_session.refresh_token.create(
...
def test_create(
    ...
    assert isinstance(session.create(version=version), expected)


def test_create_with_wrong_account(session: ig.SessionResource) -> None:
    with pytest.raises(ig.InvalidDetailsError):
        session.create(identifier="foo")


def test_create_with_wrong_password(session: ig.SessionResource) -> None:
    with pytest.raises(ig.InvalidDetailsError):
        session.create(password="foo")
...
def test_refresh_token_create(session: ig.SessionResource) -> None:
    account: t.Final = session.create()
    assert isinstance(
        session.refresh_token.create(account.oauth_token.refresh_token),
        ig.session.v1.OAuthToken,
    )
