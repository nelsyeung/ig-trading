...
async def test_async_create_clears_stale_auth_state(
    async_session: ig.AsyncSessionResource, async_requester: mock.AsyncMock
) -> None:
    # IG rejects a login outright if a stale account ID, bearer token, CST or
    # security token is attached to the request, even when the credentials
    # being submitted are otherwise valid, so `create()` must clear all of a
    # requester's previous identity before issuing the request.
    async_requester.account_id = "an-account"
    async_requester.bearer_token = "a-bearer-token"
    async_requester.cst = "stale-cst"
    async_requester.x_security_token = "stale-token"
    async_requester.post.return_value = _account_summary_v3

    await async_session.create(identifier="a-user", password="a-password")

    assert async_requester.account_id is None
    assert async_requester.bearer_token is None
    assert async_requester.cst is None
    assert async_requester.x_security_token is None
...
def test_create_clears_stale_auth_state(
    session: ig.SessionResource, requester: mock.MagicMock
) -> None:
    # See the equivalent comment in
    # `test_async_create_clears_stale_auth_state`.
    requester.account_id = "an-account"
    requester.bearer_token = "a-bearer-token"
    requester.cst = "stale-cst"
    requester.x_security_token = "stale-token"
    requester.post.return_value = _account_summary_v3

    session.create(identifier="a-user", password="a-password")

    assert requester.account_id is None
    assert requester.bearer_token is None
    assert requester.cst is None
    assert requester.x_security_token is None
...
def test_get(session: ig.SessionResource, requester: mock.MagicMock) -> None:
